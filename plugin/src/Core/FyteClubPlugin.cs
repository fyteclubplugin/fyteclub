    ...
    public sealed partial class FyteClubPlugin : IDalamudPlugin, IMediatorSubscriber
    {
        ...
        private async Task BulkApplyCachedMods()
        {
            ...
            try
            {
                ...
                if (cacheHits > 0)
                {
                    _pluginLog.Info($"⚡ BULK: Applied cached mods to {cacheHits}/{allVisiblePlayers.Count} visible players");
                }
            }
            catch (Exception ex)
            {
                _pluginLog.Warning($"Bulk cache apply failed: {ex.Message}");
            }
        }
        ...
        private async Task TryApplyCachedModsInstantly(string playerName)
        {
            try
            {
                ...
                if (_componentCache != null)
                {
                    ...
                    if (cachedRecipe != null)
                    {
                        _pluginLog.Info($"⚡ INSTANT: Cached recipe applied for {playerName}");
                        await _modSystemIntegration.ApplyPlayerMods(cachedRecipe, playerName);
                        _loadingStates[playerName] = LoadingState.Complete;
                        cacheHit = true;
                    }
                ...
                if (!cacheHit && _clientCache != null)
                {
                    ...
                    {
                        _pluginLog.Info($"⚡ INSTANT: Cached mods applied for {playerName}");
                        await ApplyPlayerModsFromCache(playerName, cachedMods);
                        _loadingStates[playerName] = LoadingState.Complete;
                        cacheHit = true;
                    }
                ...
                if (!cacheHit)
                {
                    _pluginLog.Debug($"No cache for {playerName} - will queue for P2P sync");
                }
            }
            catch (Exception ex)
            {
                _pluginLog.Warning($"Failed to apply cached mods for {playerName}: {ex.Message}");
            }
        }
        ...
        private void AddToSyncQueueIfNeeded(PlayerDetectedMessage message)
        {
            ...
            _framework.RunOnFrameworkThread(() =>
            {
                ...
                _pluginLog.Debug($"Queued {message.PlayerName} for P2P sync at {distance:F1}m");
            });
        }
        ...
        private async Task ProcessSyncQueue()
        {
            ...
            try
            {
                ...
                _pluginLog.Info($"Processing sync batch: {batch.Count} players");
                ...
                var tasks = batch.Select(async entry =>
                {
                    ...
                    {
                        _pluginLog.Warning($"Failed to sync {entry.PlayerName}: {ex.Message}");
                    }
                });
        ...
        private async Task ProcessPlayerSync(SyncQueueEntry entry)
        {
            ...
            _pluginLog.Info($"🔄 P2P Sync: {entry.PlayerName} (queued {(DateTime.UtcNow - entry.DetectedAt).TotalSeconds:F1}s ago)");
        ...
        private async Task CheckForHashChanges()
        {
            try
            {
                ...
                foreach (var kvp in _playerHashes.ToList())
                {
                    ...
                    if (!string.IsNullOrEmpty(currentHash) && currentHash != oldHash)
                    {
                        ...
                        _pluginLog.Info($"Hash changed for {playerName}: {oldHash[..8]} -> {currentHash[..8]}");
                    }
                }
            ...
            {
                _pluginLog.Warning($"Hash change check failed: {ex.Message}");
            }
        }
