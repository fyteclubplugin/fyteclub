...
namespace FyteClub
{
    ...
    public class P2PModSyncOrchestrator : IDisposable
    {
        ...
        private async Task ApplyModsFromCache(string peerId, PeerModState peerState)
        {
            ...
            try
            {
                ...
                if (result.Success)
                {
                    ...
                    _networkLogger.LogCacheOperation(sessionId, "APPLY_FROM_CACHE", peerId, "Applied from cache", new Dictionary<string, object> { ["duration"] = duration.TotalMilliseconds });
        ...
        public OrchestrationStats GetStats()
        {
            ...
            var networkStatsObj = (dynamic)networkStats;
            return new OrchestrationStats
            {
                ...
                NetworkSuccessRate = networkStatsObj.SuccessRate,
                CacheHitRate = networkStatsObj.CacheHitRate,
