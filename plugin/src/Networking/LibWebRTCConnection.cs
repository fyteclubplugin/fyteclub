...
namespace FyteClub.Networking
{
    public class LibWebRTCConnection : IWebRTCConnection
    {
        ...
        {
            // This transport never touches NNostrSignaling (Nostr signaling lives in WebRTCConnection),
            // so there is no payload here that the group key would encrypt.
            try
            {
        ...
        public async Task<string> CreateAnswerAsync(string offerSdp, byte[] groupKey)
        {
            try
            {
                if (_peerConnection == null) return string.Empty;
