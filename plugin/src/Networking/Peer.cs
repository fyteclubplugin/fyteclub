    ...
    public class Peer : IDisposable
    {
        ...
        public async Task SendDataAsync(byte[] data)
        {
            ...
            if (state == Microsoft.MixedReality.WebRTC.DataChannel.ChannelState.Open)
            {
                ...
                {
                    FyteLog.Error(LogModule.WebRTC, $"[WebRTC] Failed to send data for {PeerId}: {ex.Message}");
            ...
            else if (state == Microsoft.MixedReality.WebRTC.DataChannel.ChannelState.Connecting && DataChannelReady)
            {
                ...
                {
                    FyteLog.Error(LogModule.WebRTC, $"[WebRTC] Failed to send data for {PeerId} despite ready flag: {ex.Message}");
                    throw new Exception($"WebRTC send failed: {ex.Message}");
                }
            }
            else
            {
                ...
                FyteLog.Debug(LogModule.WebRTC, $"[WebRTC] Cannot send data for {PeerId} - channel state: {stateStr}, Ready: {DataChannelReady}");
