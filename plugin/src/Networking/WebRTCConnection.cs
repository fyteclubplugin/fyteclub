...
namespace FyteClub.Networking
{
    public class WebRTCConnection : IWebRTCConnection
    {
        ...
        private List<FyteClub.Networking.TurnServerInfo> _turnServers = new();
    ...
    private const double BUFFER_LIMIT_MARGIN = 0.85; // Keep 15% headroom beneath reported channel limit
        ...
        public List<FyteClub.Networking.TurnServerInfo> TurnServers => _turnServers;
        ...
        public WebRTCConnection(IPluginLog? pluginLog = null, string? configDirectory = null)
        {
        ...
        {
            _pluginLog?.Warning($"[WebRTCConnection] Connection disconnected - event-driven reconnection will be triggered by higher-level manager");
        ...
        public Task<bool> InitializeAsync()
        {
            ...
            try
            {
                ...
                // Placeholder signaling instance - never assigned a UUID (see SetSignalingChannel swap in
                // CreateOfferAsync/CreateAnswerAsync below), so it never actually publishes/decrypts content.
                var nostrSignaling = new NNostrSignaling(relays, priv, pub, groupKey: null, _pluginLog);
                ...
                _webrtcManager.OnPeerConnected += (peer) => {
                    ...
                    {
                        _pluginLog?.Info($" [WebRTC] Peer {peer.PeerId} already connected, skipping duplicate");
                        return;
                    }
        ...
        public async Task<string> CreateOfferAsync(byte[] groupKey)
        {
            ...
            try
            {
                ...
                _hostNostrSignaling = new NNostrSignaling(relays, priv, pub, groupKey, _pluginLog);
                ...
                _hostNostrSignaling.OnAnswerReceived += (evtUuid, answerSdp) =>
                {
                    if (evtUuid == uuid)
                    {
                        if (answerReceived)
                        {
                            _pluginLog?.Warning($"[WebRTC] Duplicate answer event for UUID {uuid}, ignoring");
                            return;
                        }
        ...
        {
            FyteLog.Debug(LogModule.WebRTC, $" [WebRTCConnection] Setting syncshell info: {syncshellId}");
            
            _currentSyncshellId = syncshellId;
            _currentPassword = password;
            
            // Save to persistence for reconnection
            FyteLog.Debug(LogModule.WebRTC, $" [WebRTCConnection] Saving syncshell to persistence");
            _persistence?.SaveSyncshell(syncshellId, password, new List<string>(), "local_peer");
            
            FyteLog.Debug(LogModule.WebRTC, $" [WebRTCConnection] Syncshell info set and saved");
        }
        
        public async Task<string> CreateAnswerAsync(string offerUrl, byte[] groupKey)
        {
            return await CreateAnswerAsync(offerUrl, groupKey, null);
        }

        public async Task<string> CreateAnswerAsync(string offerUrl, byte[] groupKey, string? answerUuid)
        {
            try
            {
                _pluginLog?.Info($"[WebRTC] JOINER: CreateAnswerAsync called with offerUrl: {offerUrl}");
                FyteLog.Debug(LogModule.WebRTC, $"[JOINER] CreateAnswerAsync called with offerUrl: {offerUrl}");

                if (_webrtcManager == null) {
                    _pluginLog?.Error($"[WebRTC] CreateAnswerAsync failed: _webrtcManager is null");
                    FyteLog.Debug(LogModule.WebRTC, $"[JOINER] _webrtcManager is null");
                    return string.Empty;
                }
                ...
                if (_currentPeer != null)
                {
                    ...
                    FyteLog.Debug(LogModule.WebRTC, $"[JOINER] Already connected to a peer, skipping offer processing");
                ...
                FyteLog.Debug(LogModule.WebRTC, $"[JOINER] Checking if offerUrl starts with nostr://");
                if (offerUrl.StartsWith("nostr://", StringComparison.OrdinalIgnoreCase))
                {
                    _pluginLog?.Info($"[WebRTC] JOINER: Parsing Nostr offer URI");
                    FyteLog.Debug(LogModule.WebRTC, $"[JOINER] Parsing Nostr offer URI");
                    ...
                    FyteLog.Debug(LogModule.WebRTC, $"[JOINER] Using Nostr relays: {string.Join(",", relays)} for uuid {uuid}");

                    _pluginLog?.Info($"[WebRTC] JOINER: Generating ephemeral keys");
                    FyteLog.Debug(LogModule.WebRTC, $"[JOINER] Generating ephemeral keys");
                    var (priv, pub) = NostrUtil.GenerateEphemeralKeys();
                    _pluginLog?.Info($"[WebRTC] JOINER: Creating NostrSignaling instance");
                    FyteLog.Debug(LogModule.WebRTC, $"[JOINER] Creating NostrSignaling instance");
                    var nostr = new NNostrSignaling(relays.Length > 0 ? relays : new[] { "wss://relay.damus.io", "wss://nos.lol", "wss://nostr-pub.wellorder.net" }, priv, pub, groupKey, _pluginLog);
                    ...
                    FyteLog.Debug(LogModule.WebRTC, $"[JOINER] Subscribing to uuid {uuid}");
                    await nostr.SubscribeAsync(uuid);
                    _pluginLog?.Info($"[WebRTC] JOINER: Subscribed, HandleOffer will process offers automatically");
                    FyteLog.Debug(LogModule.WebRTC, $"[JOINER] Subscribed, HandleOffer will process offers automatically");
                ...
                else
                {
                    ...
                    FyteLog.Debug(LogModule.WebRTC, $"[JOINER] Invalid invite code, only nostr:// supported. Input: {offerUrl}");
            ...
            catch (Exception ex)
            {
                ...
                FyteLog.Debug(LogModule.WebRTC, $"[JOINER] Exception in CreateAnswerAsync: {ex.Message}");
                FyteLog.Debug(LogModule.WebRTC, $"[JOINER] Stack trace: {ex.StackTrace}");
                throw;
            }
        }
        ...
        private void TriggerBootstrap()
        {
            ...
            {
                _pluginLog?.Info(" [WebRTC] Bootstrap already completed, skipping");
                return;
            }
            ...
            _ = SafeTask.Run(async () => {
                try {
                    ...
                    var memberRequest = System.Text.Json.JsonSerializer.Serialize(new {
                        // Was hardcoded to 10 (FileChunkMessage's ordinal, not MemberListRequest's
                        // 11) - same bug found and fixed in SyncshellManager.RequestMemberListSync,
                        // see docs/PLAN.md Phase 3 item 7.
        ...
        public void ProcessInviteWithIce(string inviteCode, byte[] groupKey)
        {
            ...
            if (parts.Length >= 3)
            {
                ...
                _ = SafeTask.Run(async () => {
                    var answer = await CreateAnswerAsync(offerUrl, groupKey);
        ...
        public void ConfigureTurnServers(List<FyteClub.Networking.TurnServerInfo> turnServers)
        {
        ...
        public void SelectTurnServerForSyncshell(string syncshellId)
        {
            ...
            {
                var serverList = new List<FyteClub.Networking.TurnServerInfo> { selectedServer };
        ...
        public void Dispose()
        {
            try
            {
                _pluginLog?.Info($"[WebRTC] Dispose() called - clearing all channels and state");
                ...
                lock (_channelLock)
                {
                    _pluginLog?.Info($"[WebRTC] Clearing {_localSendingChannels.Count} local sending channels");
                    _channels.Clear();
            ...
            {
                _pluginLog?.Error($"Error during WebRTCConnection disposal: {ex.Message}");
            }
        }
