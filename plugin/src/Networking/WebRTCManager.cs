...
namespace FyteClub.Networking
{
    public class WebRTCManager : IDisposable
    {
        ...
        {
            // STUN-first with a public TURN fallback (openrelay.metered.ca) for NAT traversal.
            // No project-hosted TURN: see docs/PLAN.md AD-1 for the planned configurable
            // user-supplied TURN server support.
        ...
        public async Task<string> CreateAnswerAsync(string peerId, string offerSdp)
        {
            ...
            _ = SafeTask.Run(async () => await ProcessPendingIceCandidates(peerId), LogModule.WebRTC);
    ...
    public string ProcessOffer(string offerUrl, string peerId)
        ...
        private async Task<Peer> CreatePeer(string peerId, bool isOfferer)
        {
            ...
            lock (_peers)
            {
                if (_peers.ContainsKey(peerId))
                {
                    _pluginLog?.Warning($"[WebRTC] Peer {peerId} already exists, returning existing peer");
                    return _peers[peerId];
                }
            }
            
            _pluginLog?.Info($"[WebRTC] Creating peer {peerId}, isOfferer: {isOfferer}");
            ...
            {
                _pluginLog?.Warning($"[WebRTC] Peer {peerId} was created concurrently. Disposing this duplicate and returning existing.");
            ...
            peerConnection.DataChannelAdded += (channel) => {
                try
                {
                    ...
                    if (_disposed) 
                    {
                        _pluginLog?.Warning($"[WebRTC] DataChannelAdded fired but manager disposed for {peer.PeerId}");
                        return;
                    }
        ...
        private async void HandleOffer(string peerId, string offerSdp)
        {
            try
            {
                ...
                if (_peers.ContainsKey(peerId))
                {
                    ...
                    _pluginLog?.Info($"[WebRTC] Re-offer received for existing answerer peer {peerId} - applying and answering");
        ...
        public async void HandleAnswer(string peerId, string answerSdp)
        {
            try
            {
                ...
                if (_peers.TryGetValue(peerId, out var peer))
                {
                    ...
                    if (!peer.IsOfferer)
                    {
                        _pluginLog?.Warning($"[WebRTC] Received answer for non-offerer peer {peerId}, ignoring");
                        return;
                    }
                    ...
                    {
                        _pluginLog?.Info($"[WebRTC] Answer already processed for {peerId}, ignoring duplicate");
                        return;
                    }
                ...
                else
                {
                    ...
                    _pluginLog?.Warning($"[WebRTC] NO PEER FOUND for answer from {peerId} - answer arrived before offer was processed");
                }
            }
            catch (Exception ex)
            {
