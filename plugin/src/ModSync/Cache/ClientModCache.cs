...
namespace FyteClub.ModSync.Cache
{
    ...
    public class ClientModCache : IDisposable
    {
        ...
        public ClientModCache(IPluginLog pluginLog, string pluginDir)
        {
            ...
            InitializeCache();
            _ = SafeTask.Run(async () => await LoadCacheManifest(), LogModule.Cache);
        ...
        public async Task CachePlayerMods(string playerId, byte[] modData, string serverTimestamp)
        {
            try
            {
                ...
                _ = SafeTask.Run(async () => await CleanupIfNeeded(), LogModule.Cache);
            }
            catch (Exception ex)
            {
        ...
        public void UpdateRecipeForPlayer(string playerName, object recipeData)
        {
            try
            {
                ...
                else
                {
                    ...
                    _ = SafeTask.Run(async () => await CreateCacheEntryFromRecipe(playerName, recipeData), LogModule.Cache);
                }
            }
            catch (Exception ex)
            {
