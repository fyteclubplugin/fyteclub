...
namespace FyteClub.ModSync.Protocol
{
    ...
    AppearanceUpdate,
        ...
        Rekey // Host-signed key-epoch advance, broadcast on member removal
    }
    ...
    public abstract class P2PModMessage
    {
        ...
        public int ProtocolVersion { get; set; } = P2PModProtocol.CurrentProtocolVersion;
    ...
    public class MemberListResponseMessage : P2PModMessage
    {
        ...
        public int KeyEpoch { get; set; }
        public RekeyMessage? CatchUpRekey { get; set; }
    }

    /// <summary>
    /// Host-signed key-epoch advance. Broadcast to remaining members when the host removes
    /// someone, and replayed as MemberListResponseMessage.CatchUpRekey for anyone who reconnects
    /// behind. Signature is verified against SyncshellInfo.HostPeerId, pinned at join time from
    /// the invite - never against a wire-supplied host name claim.
    /// </summary>
    public class RekeyMessage : P2PModMessage
    {
        public RekeyMessage() { Type = P2PModMessageType.Rekey; }

        public string SyncshellId { get; set; } = string.Empty;
        public int NewEpoch { get; set; }
        public byte[] NewEncryptionKey { get; set; } = Array.Empty<byte>();
        public string RemovedMemberName { get; set; } = string.Empty;
        public byte[] HostSignature { get; set; } = Array.Empty<byte>();

        /// <summary>
        /// Deterministic byte layout signed by the host and re-verified by every recipient.
        /// Length-prefixed (not delimiter-joined) so no field's content can be crafted to shift
        ...
        public byte[] BuildSigningPayload()
        {
            ...
            writer.Write(SyncshellId);
            writer.Write(NewEpoch);
            writer.Write(NewEncryptionKey.Length);
            writer.Write(NewEncryptionKey);
            writer.Write(RemovedMemberName);
    ...
    /// <summary>
    /// One file's identity in a manifest - hash-addressed, no content. Lets a receiver diff
    /// against its own cache before anything is transferred (docs/PLAN.md Phase 3 item 4, AD-7.1).
    /// </summary>
    ...
    /// <summary>
    /// Lightweight appearance update sent ahead of full mod transfers. Also carries a
    /// hash-only manifest of the sender's files (docs/PLAN.md Phase 3 item 4, AD-7.1): the
    /// receiver diffs FileManifest against its own content-addressed cache and pulls only
    /// what's missing via ComponentRequest, rather than the sender pushing everything to every
    /// peer regardless of what they already have. SourcePeerId is stamped by the receiving
    /// orchestrator (P2PModSyncOrchestrator.ProcessIncomingMessage), not sent over the
    /// wire by the sender - it identifies which peer connection to route a ComponentRequest back
    /// through.
    /// </summary>
    public class AppearanceUpdateMessage : P2PModMessage
    {
        ...
        public List<FileManifestEntry> FileManifest { get; set; } = new();
    ...
    public class ChannelNegotiationMessage : P2PModMessage
    {
        ...
        /// <summary>
        /// Optional-feature names this client understands (docs/PLAN.md Phase 3 item 3), so
        /// future capabilities can be negotiated without another protocol version bump. Empty
        /// today - no optional capabilities exist yet.
        /// </summary>
    ...
    public class ChannelNegotiationResponse : P2PModMessage
    {
        ...
        /// <summary>
        /// Optional-feature names this client understands - see ChannelNegotiationMessage.Capabilities.
        /// </summary>
        public List<string> Capabilities { get; set; } = new();
    }
    ...
    {
        /// <summary>
        /// Current envelope version (docs/PLAN.md Phase 3 item 3, AD-6). Bump this whenever a
        /// message shape or P2PModMessageType member is added/changed in a way older clients
        /// can't parse. A message with no "protocolVersion" field (every message before this
        /// version existed) is treated as version 1 - the version this field was introduced at.
        /// </summary>
        public const int CurrentProtocolVersion = 1;

        private static int _versionMismatchCount;
        public static int VersionMismatchCount => _versionMismatchCount;
    ...
    public event Func<AppearanceUpdateMessage, Task>? OnAppearanceUpdateReceived;
        ...
        public event Func<RekeyMessage, Task>? OnRekeyReceived;
            ...
            try
            {
                ...
                using (document)
                {
                    ...
                    if (!document.RootElement.TryGetProperty("type", out var typeElement))
                    {
                        ...
                        {
                            _pluginLog.Warning("[P2P] Detected 'Type' property (PascalCase). Proceeding with it.");
...
if (typeElement.ValueKind == JsonValueKind.Number)
{
    ...
    if (!Enum.IsDefined(typeof(P2PModMessageType), typeInt))
    {
        ...
        WarnIfVersionMismatch(document.RootElement);
        return null;
    }
...
else if (typeElement.ValueKind == JsonValueKind.String)
{
    ...
    switch (lowered)
    {
        ...
        case "memberlistresponse":
            ...
            break;
        // "mod_sync_request"/"modsyncrequest" deliberately NOT mapped here: the legacy
        // sender's intent is "please send me sync data" (SyncshellManager.HandleModSyncRequest
        // proactively pushes cached mod data back), but this enum value means the opposite —
        // "here is data, apply it" (P2PModSyncOrchestrator.HandleModApplicationRequest
        // calls ApplyPlayerMods). Mapping it here would deserialize a malformed
        // ModApplicationRequest with a blank TargetPlayerName and empty FileReplacements.
        // Falls through to the default case below and is handled exclusively by
        // SyncshellManager.HandleModData, same as "phonebook_request".
        case "apply_mods":
        ...
        default:
            ...
            {
                _pluginLog.Warning($"[P2P] Unsupported string type '{rawType}', attempting legacy parse bypass");
    ...
    // The switch above resolves legacy aliases (e.g. "member_list_request") to the right
    // enum value for routing, but the JSON string still contains the original alias text.
    // JsonStringEnumConverter only recognizes exact enum member names, so every downstream
    // Deserialize<T> call below would fail on the alias even with the converter present.
    // Normalize the "type" field to its numeric form — which always deserializes correctly,
    // converter or not — before any further parsing.
    try
    {
        var normalizedNode = System.Text.Json.Nodes.JsonNode.Parse(json)!.AsObject();
        normalizedNode["type"] = (int)messageType;
        json = normalizedNode.ToJsonString();
    }
    catch (Exception normEx)
    {
        _pluginLog.Warning($"[P2P] Failed to normalize type discriminator for '{rawType}': {normEx.Message}");
    }
}
else
{
    ...
    return null;
                    ...
                    return messageType switch
                    {
                        ...
                        P2PModMessageType.Rekey => JsonSerializer.Deserialize<RekeyMessage>(json, options),
                        _ => null
                    };
                }
            }
        ...
        public async Task ProcessMessage(P2PModMessage message)
        {
            try
            {
                ...
                switch (message)
                {
                    ...
                    case RekeyMessage rekey:
                        if (OnRekeyReceived != null)
                        {
                            await OnRekeyReceived(rekey);
                        }
                        break;
        ...
        /// <summary>
        /// Called when a message's "type" value isn't recognized by this build. If the message
        /// also carries a "protocolVersion" higher than we support, that's the likely cause - the
        /// sender is running a newer FyteClub that added a message type we don't know about yet.
        /// Logs a distinct, clearly-labeled warning (not just another generic parse-failure line)
        /// and increments a UI-visible counter (docs/PLAN.md Phase 3 item 3), instead of the type
        /// mismatch silently looking like any other malformed-message failure.
        /// </summary>
        private void WarnIfVersionMismatch(JsonElement root)
        {
            if (root.TryGetProperty("protocolVersion", out var versionElement) &&
                versionElement.ValueKind == JsonValueKind.Number)
            {
                var remoteVersion = versionElement.GetInt32();
                if (remoteVersion > CurrentProtocolVersion)
                {
                    Interlocked.Increment(ref _versionMismatchCount);
                    _pluginLog.Warning($"[P2P] VERSION MISMATCH: peer is using protocol v{remoteVersion}, this client supports v{CurrentProtocolVersion}. Update FyteClub to sync with this peer - some messages will be silently unsupported until then.");
                }
            }
        }
        ...
        /// </summary>
        public static string CalculateDataHash(PlayerInfo playerInfo, Dictionary<string, TransferableFile> files)
        {
